"use client";
import { TodoListElement, TodoListElementProps } from "@/components/ui";
import { nanoid } from "nanoid";
import { FormEvent, useRef, useState } from "react";

enum TodoListErrorEnum {
  EMPTY_INPUT_ERROR = "Пустое поле ввода",
}

export default function TodoPage() {
  const [list, setList] = useState<TodoListElementProps[]>([]);
  ...
  return (
    <div className="flex-col pt-4 max-w-96 mx-auto">
      ...
      <div className="flex flex-col gap-2 py-3">
        {list.map((item) => {
          return (
            <TodoListElement key={item.id} id={item.id} title={item.title} />
          );
        })}
      </div>
    </div>
